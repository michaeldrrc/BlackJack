...
class Player(object):
    ...
    # number of players under 21 points - used for dealer card calculations
    number_of_players_under_21_points = 0

    #A function that initializes the class
    def __init__(self, name):
        self.name = name  # player's name as an argument
        self.list_of_cards = []  # player's list of cards
        self.money = 100  # player's money
        self.bet = 0   # player's current bet
        self.hasBlackJack = False # player has blackjack pair
    ...
    def Initialize(self):
        self.list_of_cards = []  # player's list of cards
        self.bet = 0   # player's current bet
        self.number_of_players_under_21_points = 0
        self.hasBlackJack = False # player has blackjack pair
...
class Deck_of_cards(object):
    ...
    #A function that does the initial constrction of the deck
    def construct_ordered_deck(self):
        del self.ordered_deck[:]
        i1 = 0
        j1 = len(self.Cards)
        j2 = len(self.Suits)
        # card is selected here
        while i1 < j1:
            i2 = 0
            # suit is selected here
            while i2 < j2:
                # Whole card is assembled and appended to the deck
    ...
    def construct_random_deck(self):
        ...
        while i < max:
            # j is random index for ordered deck
...
def calculate_winnings(p1, D):
    ...
    #Player has blackjack and dealer does not.  Player wins the round
    ...
    #Player and dealer have blackjack.  Bet is pushed
    elif p1.hasBlackJack == True and D.hasBlackJack == False:
        print p1.name, "push bet with dealer"

    #Player loses the round because the value of its cards are over 21
    elif p1_card_points > 21:
        p1.money -= p1.bet
        print p1.name, "lost", p1.bet, "dollars"

    #Player wins the round because the value of the dealer's cards is over 21
    elif D_card_points > 21:
        print "Dealer went over 21"
        p1.money += p1.bet
        print p1.name, "won", p1.bet, "dollars"

    #Player wins the round because the value of its cards is greater than the dealers
    elif p1_card_points > D_card_points:
        p1.money += p1.bet
        print p1.name, "won", p1.bet, "dollars"

    #Player loses the round because the value of its cards is lower than the dealers
    elif p1_card_points < D_card_points:
        p1.money -= p1.bet
        print p1.name, "lost", p1.bet, "dollars"
    else:
    ...
    # Your mother will help you if you run out of money
    if p1.money < 1:
        p1.money = 25
...
#initalize players variables for the round
continue_playing = 'y'
